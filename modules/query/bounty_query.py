...
from utils import web3obj, read_json, to_checksum_address,read_impermax_pairs_address
import os
import logging
...
class BountyQuery:
    def __init__(self):
        ...
        self.test_contract = self.web3.eth.contract(
            address=to_checksum_address('0x899caad2d29eff3b628503a85dae736b5c4d1671'), abi=self.abi)
        self.all_pair = read_json('address/contracts.json')["IMPERMAX"].keys()
    ...
    def get_bounty(self, pair):
        address = read_impermax_pairs_address(pair)
        ...
        bounty = float(reward * reinvest_rate)
        return bounty

    def get_all_bounties(self):
        bounties = {}
        for pair in self.all_pair:
            bounties[pair] = self.get_bounty(pair)
        return bounties
